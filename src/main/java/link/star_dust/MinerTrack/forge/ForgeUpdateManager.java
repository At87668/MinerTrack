...
/** Forge update-check shim. Mirrors FabricUpdateManager. */
public class ForgeUpdateManager {
    private final ForgeAdapter adapter;
    private final UpdateManagerCore core;

    public ForgeUpdateManager(ForgeAdapter adapter, DetectionBridge detectionBridge) {
        this.adapter = adapter;
        UpdateConfigSource source = new DetectionBridgeConfigSource(detectionBridge, adapter);
        this.core = new UpdateManagerCore(source, adapter.getVersion(), "forge");
    }
    ...
    private static final class DetectionBridgeConfigSource implements UpdateConfigSource {
        ...
        DetectionBridgeConfigSource(DetectionBridge bridge, ForgeAdapter adapter) {
            this.bridge = bridge;
            this.adapter = adapter;
        }
