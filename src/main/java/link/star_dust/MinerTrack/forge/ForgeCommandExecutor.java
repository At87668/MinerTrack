...
/**
 * Forge command executor. Delegates to MinerTrackCommandCore.
...
public class ForgeCommandExecutor {
    ...
    public ForgeCommandExecutor(ForgeAdapter adapter,
                                  ForgeLanguageBridge langBridge,
                                  ViolationManagerBridge vlBridge,
                                  ForgeUpdateManager updateManager,
    ...
    private MinerTrackCommandCore buildCore(Object source) {
        CommandBridge cmdBridge = new ForgeCommandBridge(source, vlBridge.getVerbosePlayers());
    ...
    private static Object playerByUuid(UUID uuid) {
        ...
        return FabricReflection.call(pm, "getPlayerByUUID",
            new Class<?>[]{UUID.class}, new Object[]{uuid});
    }
    ...
    private static Object playerByName(String name) {
        ...
        Object pm = FabricReflection.callMigrated(server, "getPlayerList", "getPlayerManager",
            FabricReflection.NO_PARAMS, FabricReflection.NO_ARGS);
        if (pm == null) return null;
        return FabricReflection.call(pm, "getPlayerByName",
            new Class<?>[]{String.class}, new Object[]{name});
    }
        ...
        public List<String> getOnlinePlayerNames() {
            ...
            try {
                Object server = commandSource != null
                    ? FabricReflection.callAny(commandSource, "getServer",
                        FabricReflection.NO_PARAMS, FabricReflection.NO_ARGS)
                    : server();
                if (server == null) return names;
                Object pm = FabricReflection.callMigrated(server, "getPlayerList",
                    "getPlayerManager", FabricReflection.NO_PARAMS, FabricReflection.NO_ARGS);
                if (pm == null) return names;
                Object players = FabricReflection.callAny(pm, "getPlayers",
                    FabricReflection.NO_PARAMS, FabricReflection.NO_ARGS);
                if (players instanceof List) {
                    for (Object p : (List<?>) players) {
                        Object name = FabricReflection.callAny(p, "getName",
                            FabricReflection.NO_PARAMS, FabricReflection.NO_ARGS);
                        String s = FabricReflection.readString(name);
    ...
    private class KickBridgeImpl implements MinerTrackCommandCore.KickBridge {
        ...
        public void kickPlayer(UUID playerId, String reason) {
            try {
                ...
                Object text = FabricReflection.createText(reason);
                if (text == null) return;
                // Get connection via ServerPlayer.connection
                Object connection = FabricReflection.callAny(player, "connection",
                    FabricReflection.NO_PARAMS, FabricReflection.NO_ARGS);
                if (connection == null) return;
                Class<?> textCls = FabricReflection.resolveTextComponentClass();
                if (textCls == null) return;
                // Try disconnect(Component) on ServerCommonPacketListenerImpl first,
                // then fall back to ServerGamePacketListenerImpl
                try {
                    FabricReflection.callAny(connection, "disconnect",
                        new Class<?>[]{textCls}, new Object[]{text});
                } catch (Throwable t) {
                    FabricReflection.invokeBySigOrThrow(connection,
                        new Class<?>[]{textCls}, new Object[]{text});
                }
            } catch (Throwable t) {
    ...
    private class ConfigReloadBridgeImpl implements MinerTrackCommandCore.ConfigReloadBridge {
        ...
        public void reloadConfig() {
            ...
            if (detectionBridge instanceof ForgeDetectionBridge) {
                ((ForgeDetectionBridge) detectionBridge).loadGroupConfigs();
            }
            violationManager.reloadConfig();
            langBridge.reloadLanguage();
            ForgeReflection.clearDebugCache();
        }
    }
    ...
    private class LogViewerBridgeImpl implements MinerTrackCommandCore.LogViewerBridge {
        @Override
        public List<String> getRecentLogLines(int count) {
            List<String> lines = new ArrayList<>();
            try {
                File logFile = new File(adapter.getDataFolder(), "violations.log");
                if (!logFile.exists()) return lines;
                List<String> all = Files.readAllLines(logFile.toPath());
                int start = Math.max(0, all.size() - count);
                for (int i = start; i < all.size(); i++) lines.add(all.get(i));
        ...
        public void clearLog() {
            try {
                File logFile = new File(adapter.getDataFolder(), "violations.log");
                if (logFile.exists()) Files.write(logFile.toPath(), new byte[0]);
