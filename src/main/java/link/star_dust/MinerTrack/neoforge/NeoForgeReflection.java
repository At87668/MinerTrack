...
final class NeoForgeReflection {
    ...
    private static Method findMethodImpl(Class<?> cls, String name, Class<?>[] paramTypes) { if (cls == null || name == null) return null; MethodKey key = new MethodKey(cls, name, paramTypes); Object cached = methodCache.get(key); if (cached != null) return unwrap(cached); String resolved = NeoForgeReflectionConstants.redirectMethod(name); Method m = tryMethod(cls, resolved, paramTypes); if (m != null) { methodCache.put(key, m); return m; } if (!resolved.equals(name)) { m = tryMethod(cls, name, paramTypes); if (m != null) { methodCache.put(key, m); return m; } } m = scanMethodNamed(cls, resolved, paramTypes); if (m != null) { methodCache.put(key, m); return m; } if (!resolved.equals(name)) { m = scanMethodNamed(cls, name, paramTypes); if (m != null) { methodCache.put(key, m); return m; } } Class<?> sup = cls.getSuperclass(); if (sup != null && sup != Object.class) { m = findMethodImpl(sup, name, paramTypes); if (m != null) { methodCache.put(key, m); return m; } } if (DEBUG_REFLECTION) log("M-MISS " + cls.getName() + "." + name + " (resolved=" + resolved + ")"); methodCache.put(key, NOT_FOUND); return null; }
