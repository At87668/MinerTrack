...
/**
 * Self-contained reflection helpers for accessing NeoForge internals.
 *
 * <p>NeoForge 20.2+ uses Mojang names at runtime for both class and method names.
 ...
 *
 * <p>NeoForge uses {@code net.neoforged.*} package prefix for its APIs,
 * whereas Forge uses {@code net.minecraftforge.*}.
 */
final class NeoForgeReflection {
    ...
    // ==================================================================
    // Class loading (NeoForge: Mojang names work directly at runtime)
    // ==================================================================
    ...
    // ==================================================================
    // NeoForge config directory (net.neoforged.* instead of net.minecraftforge.*)
    // ==================================================================

    static java.nio.file.Path getConfigDir() {
        try { Class<?> fmlPaths = neoClass("net.neoforged.fml.loading.FMLPaths"); if (fmlPaths == null) return java.nio.file.Path.of("config"); Field f = fmlPaths.getField("CONFIGDIR"); Object v = f.get(null); return v instanceof java.nio.file.Path ? (java.nio.file.Path) v : java.nio.file.Path.of("config"); }
        catch (Throwable t) { return java.nio.file.Path.of("config"); }
    }

    static String getModVersion(String modId) {
        try {
            Class<?> modListCls = neoClass("net.neoforged.fml.ModList"); if (modListCls == null) return "unknown";
    ...
    // ==================================================================
    // NeoForge event bus (NeoForge.EVENT_BUS instead of MinecraftForge.EVENT_BUS)
    // ==================================================================

    static Object getMainEventBus() {
        try { Class<?> neoForge = neoClass("net.neoforged.neoforge.common.NeoForge"); if (neoForge == null) return null; Field eb = neoForge.getField("EVENT_BUS"); return eb.get(null); }
        catch (Throwable t) { return null; }
    }
